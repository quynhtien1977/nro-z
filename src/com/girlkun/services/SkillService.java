...
//    }
    public boolean useSkill(Player player, Player plTarget, Mob mobTarget, Message message) 
    {
        boolean flagcheck = player.getSession() != null && player.zone != null && player.zone.map != null && player.zone.map.mapId == 57 && player.location != null && player.location.x >= 755 && player.location.x >= 1070;
        if (player.effectSkill.isHaveEffectSkill()) {
            return false;
        }
        ...
        if (player.effectSkill.isHaveEffectSkill()) {
            if (player.playerSkill.skillSelect.template.id != Skill.TU_SAT
                    && player.playerSkill.skillSelect.template.id != Skill.QUA_CAU_KENH_KHI
                    && player.playerSkill.skillSelect.template.id != Skill.MAKANKOSAPPO) 
            { 
                return false;
        ...
        if (System.currentTimeMillis() -  player.playerSkill.skillSelect.lastTimeUseThisSkill < player.playerSkill.skillSelect.coolDown ) 
        {
            return false;
        }
        if (player.effectSkill.useTroi) {
    ...
    private void userSkillSpecial(Player player, Message message) {
        ...
        //fix chọn skill 9 nhưng chưa thấy khác biệt
    //     byte st = -1;
    //     byte skillId = -1;
    //     Short dx = -1;
    //     Short dy = -1;
    ...
    // private void userSkillSpecial(Player player, Player plTPlayer, byte st, byte skillId, Short dx, Short dy, byte dir, Short x,Short y) {
    //         switch (skillId) {
    //             case Skill.SUPER_KAME:
    //                 sendEffSkillSpecialID24(player, dir);
    //                 break;
        ...
        try {
            ...
            if (player.skillSpecial.skillSpecial.template.id == Skill.MA_PHONG_BA) {
                if (player.skillSpecial.stepSkillSpecial == 0 && Util.canDoWithTime(player.skillSpecial.lastTimeSkillSpecial, SkillSpecial.TIME_GONG)) {
                    ...
                    for (Player playerMap : player.zone.getPlayers()) { // cho hut boss thi player.zone.getHumanoids()
...
//                    mobTarget.attackMob(player, false, false);
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        if (plAtt.effectSkill != null && plAtt.effectSkill.isCaiBinhChua) {
            miss = true;
        }
